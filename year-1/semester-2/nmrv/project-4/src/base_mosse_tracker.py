...
class BaseMOSSETracker(Tracker):
    ...
    def __init__(self):
        ...
        self.average_init_time = 0
        self.average_tracking_time = 0

        #self.parameters = MOSSEParams()
        super().__init__(params=MOSSEParams())
    ...
    def initialize(self, image: np.ndarray, region: np.ndarray):
        ...
        self.average_init_time += end - start + 1e-9
        self.init_frames += 1
        print(f'Initialization FPS: {self.init_frames / self.average_init_time}')
    ...
    def track(self, image: np.ndarray) -> np.ndarray:
        ...
        self.average_tracking_time += end - start
        self.tracking_frames += 1
        print(f'Tracking FPS: {self.tracking_frames / self.average_tracking_time}')

        #print(f"Tracking time of frame {self.frame}: {end - start}")
