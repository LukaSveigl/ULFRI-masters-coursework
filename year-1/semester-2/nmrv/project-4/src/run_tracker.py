...
#from base_mosse_tracker import BaseMOSSETracker, MOSSEParams
from particle_tracker import ParticleTracker, ParticleParams

# set the path to directory where you have the sequences
dataset_path = './data/' # set to the dataset path on your disk
...
#tracker = BaseMOSSETracker(parameters)
#tracker = BaseMOSSETracker()
...
while frame_idx < sequence.length():
    ...
    if frame_idx == init_frame:
        ...
        for i in range(len(particles)):
            ...
            color = (0, 255, 0)
            cv2.circle(img, (int(round(particles[i, 0])), int(round(particles[i, 1]))), round(weights[i] * 50), color, -1)
            #cv2.circle(img, (int(round(particles[i, 0])), int(round(particles[i, 1]))), 1, color, -1)
