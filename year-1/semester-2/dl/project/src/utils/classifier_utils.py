...
def get_optimizer(config: Dict[str, any], classifier: nn.Module) -> torch.optim.Optimizer:
    ...
    if config['optimizer'] == 'adam':
        ...
        if 'learning_rate' not in config or 'weight_decay' not in config:
            raise ValueError('Learning rate not specified for SGD optimizer')
        optimizer = torch.optim.SGD(classifier.parameters(), lr=config['learning_rate'], weight_decay=config['weight_decay'])
    else:
